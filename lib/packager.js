	...
	return new Promise((resolve, reject) => {
		...
		Object.keys(targets).forEach(target => {		
			...
			masterPromises.push(
				...
				copyDirectory(zipTraffic, filePath, filterExcludedFile, 'traffic_source/', true).then(() => {
...
function copyDirectory(zip, dirBase, filter, copyDestination, trace = false) {
	return new Promise((resolve, reject) => {
		walk.walk(dirBase)
			.on('file', (root, stat, next) => {	
				if (filter.call(this, stat.name)) {
					...
					fs.readFile(root + '/' + stat.name, (err, data) => {	
						...
						if (trace) log('file:', stat.name, '| root:', root, '| baseFolder:', baseFolder)
