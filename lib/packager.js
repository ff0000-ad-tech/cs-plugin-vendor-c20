	...
	return new Promise((resolve, reject) => {
		...
		Object.keys(targets).forEach(target => {		
			...
			masterPromises.push(
				...
				copyDirectory(zipTraffic, filePath).then(() => {
			...
			let containerPromises = [
				// common/
				copyDirectory(zipContainer, './1-build/common/', true),
				// [size]/
				copyDirectory(zipContainer, './1-build/' + folderName + '/', true)
			]
...
function copyDirectory(zip, dirCommonBase, copyDestination = false) {
	return new Promise((resolve, reject) => {
		walk.walk(dirCommonBase)
			.on('file', (root, stat, next) => {									
				if (excludedFiles.includes(stat.name)) {
					...
					fs.readFile(root + '/' + stat.name, (err, data) => {	
						...
						let baseFolder = String(root).substr(dirCommonBase.length) + '/'
						...
						zip.file((copyDestination ? dirCommonBase : '') + baseFolder + stat.name, data)
						next()
					})
				}
			})
